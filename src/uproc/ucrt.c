...
PUBLIC	s32	sys_exit(s32 pid)
{
	...
	print_memflist();	
	printk("Originally by ZhaoFeng Liang <zhf.liang@outlook.com>\n");	
	printk("time is %d-%d-%d %d:%d:%d,(tick:%d)", sys_ntime->year, sys_ntime->mon, sys_ntime->day, sys_ntime->hour, sys_ntime->min, sys_ntime->sec, ticket);
}
...
PUBLIC	s32	sys_execl(struct proc *new_p, s8 *filename, s32 argc, u32 argv_addr)
{
	...
	// read exe file into 0x700000(p_tmp_data);
