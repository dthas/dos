...
//
// Attentiion:
// This file is used for segmentation with paging. And all the "bin files"(
// such as echo, cat, mkdir etc) are need to compile with -Ttext 0x600000 
//===========================================================================
...
PUBLIC	s32	sys_execl(struct proc *new_p, s8 *filename, s32 argc, u32 argv_addr)
{
	...
	u32	data_base_vaddr	= USER_PROC_RESERVE + code_size;
	...
	s32 k = (USER_PROC_RESERVE + total_size) / PAGE_SIZE + 1;// k is the total bits that use to remark the status of each page
	...
	new_p->regs.esp 	= USER_PROC_RESERVE + total_size;
	new_p->regs.eip 	= USER_PROC_RESERVE;			//6MB
	new_p->mem_paddr	= new_p_base;

	new_p->mem_size_bytes	= USER_PROC_RESERVE + total_size;
	
	u32 index		= new_p->pid;
	upd_gdt_descs_user(index, new_p_base);
	...
	*(u32*)(paddr)		= new_p_base;
	...
	u32	base_laddr	= desc_baseaddr_get(new_p->pid) + USER_PROC_RESERVE; 
